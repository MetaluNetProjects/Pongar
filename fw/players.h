...
inline uint16_t deg_from_size(uint16_t size, uint16_t distance) { return size * 180.0 / (distance * 3.14159); }
inline uint16_t size_from_deg(uint16_t angle, uint16_t distance) { return (angle * distance * 3.14159) / 180.0; }
...
struct Position {
    ...
    uint16_t size;      // mm
    inline void set_size_deg(int deg) { size = size_from_deg(deg, distance); } //deg * 3.14159 / 180.0) * distance; }
    inline int get_size_deg() { return deg_from_size(size, distance); } //size * 180.0 / (distance * 3.14159); }
    inline void set(Position& p) { angle = p.angle; distance = p.distance; size = p.size; }
...
class Element : public Object {
  ...
  public:
    inline void associate(int player) { associated_player = player; if(player != -1) association_time = get_absolute_time(); }
    inline int get_associate() { return associated_player; }
};

class Player : public Object {
    std::set<int> elements;
  public:
    void add_element(int element) { elements.insert(element); }
    void remove_element(int element) { elements.erase(element); }
    const std::set<int>& get_elements() { return elements; }
};
...
class Players {
  ...
  public:
    ...
    const std::set<int> &get_object_set() { return objects_set; }
    bool is_object_visible(int el) { return objects[el].visible(); }
