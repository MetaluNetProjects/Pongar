...
void DmxMaster::init(uart_inst_t *uart_inst, int tx_pin_num) {
    uart = uart_inst;
    tx_pin = tx_pin_num;
...
void DmxMaster::alarm() {
    if(state == BREAK) {
        set_break(false);
        state = MAB;
        add_alarm_in_us(12, alarm_callback, this, true);
    } else {
        //sleep_us(10);
...
bool DmxMaster::transfer_frame(const unsigned char *frame, int count) {
    ...
    state = BREAK;
    set_break(true);
    add_alarm_in_us(100, alarm_callback, this, true);
    return true;
}
...
void DmxMaster::reset() {
    state = IDLE;
}

void DmxMaster::status() {
    printf("l dmx dmachan=%d state=%d\n", dma_chan, state);
}
